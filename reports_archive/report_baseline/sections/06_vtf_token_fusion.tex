...
\subsection{CLIP ViT Visual Tokens}

CLIP ViT-B/32 将 $224\times224$ 图像分为 $7\times7$ 个 patch（每 patch $32\times32$），经 ViT encoder 处理后输出 50 个 visual tokens（1 个 CLS token + 49 个 patch tokens），每个维度为 512：$F_{\mathrm{vis}} \in \mathbb{R}^{B\times50\times512}$。CLS token 聚合了全局语义，patch tokens 编码了局部区域特征。

\subsection{EEG Tokenizer}

A2 encoder 输出的 pooled EEG embedding 为单一 512 维向量。为使其与 visual tokens 进行 token-level 交互，设计了轻量 EEG tokenizer：通过 4 个并行的可学习线性投影器将 pooled embedding 扩展为 4 个 EEG tokens（$F_{\mathrm{eeg}} \in \mathbb{R}^{B\times4\times512}$）。选择 4 个 tokens 是效率与信息多样性的折中：token 过少缺乏交互多样性，过多则可能过度主导视觉表示。

\subsection{Visual-to-EEG Cross-Attention}

VTF 的核心是 visual-to-EEG cross-attention，结构如图~\ref{fig:vtf} 所示。visual tokens 作为 Query，EEG tokens 作为 Key 和 Value：
