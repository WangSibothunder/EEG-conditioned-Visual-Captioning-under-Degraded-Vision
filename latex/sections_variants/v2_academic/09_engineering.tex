...
在实际开发中遇到并解决了以下问题：(1) 增强视觉 token 的维度与 Qwen2-VL 的输入期望不完全匹配，需在 VTF 后插入本设计实现的轻量 prefix adapter 进行格式对齐和维度映射；(2) 部分 BLIP 伪 caption 存在重复、过长或与类别不一致的问题，通过最小长度过滤（$\geq$ 3 词）和正则去噪处理；(3) 全量测试涉及 1997 $\times$ 6 $\times$ 4 $\approx$ 48000 次推理，生成式模型的自回归解码是主要时间瓶颈，完整评估约需 2--4 小时；(4) 主要训练均在单张 48 GB GPU 上完成，未使用 DeepSpeed 或多 GPU 分布式训练。
