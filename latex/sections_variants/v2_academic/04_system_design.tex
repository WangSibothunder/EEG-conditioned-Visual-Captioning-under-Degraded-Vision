...
\textbf{第二阶段：A2 EEG 语义融合}。设计并实现 A2 temporal-spectral-spatial EEG encoder，从 EEG 信号中提取时间、频谱和空间三方面的语义特征。在 CLIP 语义空间中，通过门控残差融合将 EEG embedding 与退化图像的视觉 embedding 进行融合，以分类损失为主训练目标，并设置 shuffled EEG（打乱脑电）和 random EEG 两种对照，严格验证 EEG 语义信息的真实性。
